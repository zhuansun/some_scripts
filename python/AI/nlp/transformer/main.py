...
"""
    因为我们是使用Transformer进行翻译任务
    所以Translation这个类是对模型做了一层封装, 方便训练和推理
"""
class Translation(object):
    """
        初始化
    """
    def __init__(self, file="/Users/zhuansunpengcheng/workspace/github/some_scripts/python/AI/nlp/transformer/data.txt"):
        self.file = file
        self.device = "cuda:0" if torch.cuda.is_available() else "cpu"
        # 获取分词器
        self.tokenizer = get_tokenizer(file=file)
        # 获取模型
        # N=2 是指Encoder和Decoder的层数
        self.model = self._get_model(self.tokenizer.input_dict_len,
                                     self.tokenizer.output_dict_len,
                                     N=2)
        # 获取训练轮数
        self.epochs = 20
        # 获取优化器
